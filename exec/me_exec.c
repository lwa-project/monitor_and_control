...
int main ( int narg, char *argv[] ) {
  ...
  while (eQuit==0) {  /* any higher value means quit */
    ...
    if (!(client_sockfd==-1)) { /* we have a connection... */
      ...
      client_recvd = read(client_sockfd,&c,sizeof(struct me_cmd_struct));
      if (client_recvd<(ssize_t)sizeof(struct me_cmd_struct)) {
        //printf("[%d/%d] Receive failed with only %d B\n",ME_ME_C,getpid(),client_recvd);
        close(client_sockfd);
        continue;
        }
