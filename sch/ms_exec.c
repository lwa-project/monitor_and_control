...
main ( int narg, char *argv[] ) {
  ...
  long int mselog_line_ctr=0; /* keeping track of number of lines in mselog */
  ...
  while ( eSummary > LWA_SIDSUM_NULL ) {
    ...
    if (!(client_sockfd==-1)) { /* we have a connection... */                          
      ...
      if ( b_valid_sid && (eSummary != LWA_SIDSUM_SHUTDWN) ) { /* if a valid subsystem and */
          ...
          switch (c.cid) {
            ...
            case LWA_CMD_SHT:
	          ...
	          LWA_mse_log( fpl, LWA_MSELOG_MTYPE_TASK, task[tqfai].ref, 
                         ...
                         task[tqfai].sid, task[tqfai].cid, task[tqfai].data, task[tqfai].datalen, &mselog_line_ctr ); 
        ...
        /* First, if this is a scheduled task (c.bScheduled=1), make sure it's not already too late */
        bAdvNoticeOK = 1;
        if (c.bScheduled) {
          gettimeofday( &tv, NULL ); /* get the current time as a timeval */
          if ( ( ( c.tv.tv_sec - tv.tv_sec )*1000 + ( c.tv.tv_usec - tv.tv_usec )/1000 ) < LWA_MS_CMD_ADV_NOTICE_MS ) {
            bAdvNoticeOK = 0; /* reject due to insufficient advance notice */   
            }
          //printf("%11ld-%11ld=%11ld | %7ld-%7ld=%7ld | %7ld | %d\n",
          //  c.tv.tv_sec, tv.tv_sec, c.tv.tv_sec -tv.tv_sec,
          //  c.tv.tv_usec,tv.tv_usec,c.tv.tv_usec-tv.tv_usec,
          //  ( c.tv.tv_sec - tv.tv_sec )*1000 + ( c.tv.tv_usec - tv.tv_usec )/1000, bAdvNoticeOK );
          }

        /* OK, now process it ... */  
        if (    (!(c.sid==LWA_SID_MCS))    /* if not for me (MCS)... */
             &&                            /* AND */  
                ( (tqfai>=tql)   ||        /* there is NO available slot in queue, or */
                  (!bAdvNoticeOK)   )      /* this is a scheduled command with insufficient advance notice */
           ){ 
          ...
          if (bAdvNoticeOK) {     
              ...
              strcpy(c.data,"Insufficient advance notice");
            }
          c.datalen = -1;
	  ...
	  LWA_mse_log( fpl, LWA_MSELOG_MTYPE_TASK, c.ref, 
        ...
        if (    (!(c.sid==LWA_SID_MCS))    /* if not for me (MCS)... */
             &&                            /* AND */  
                ( (tqfai<tql)   &&         /* there is available slot in queue, and */
                  (bAdvNoticeOK)   )       /* no problem with insufficient advance notice */
           ){ 
    ...
    if ( (eDone==1) && (task[tqp].bScheduled) ) {
      gettimeofday( &tv, NULL ); /* get the current time as a timeval */
      if ( ( ( task[tqp].tv.tv_sec - tv.tv_sec )*1000 + ( task[tqp].tv.tv_usec - tv.tv_usec )/1000 ) > 0 ) {
        eDone=2; /* not time yet */   
        }
      }
    ...
    for ( i=0; i<tql; i++ ) {
	   ...
	   LWA_mse_log( fpl, 
                        ...
                        task[i].sid, 
                        task[i].cid, 
                        "Timed out at ms_mcic", -1, &mselog_line_ctr ); 
...
//   .1: Implemented capability to request commands to execute at a specified time
