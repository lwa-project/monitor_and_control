...
#define MY_NAME "ms_init (v.20101016.1)"
#define ME "1" 
...
int main ( int narg, char *argv[] ) {
  ...
  printf("[%s] Launching ms_mdre_ip...\n",ME);
  
  /* Launch ms_mdre_ip */
  ms_mdre_pid = fork();               /* create duplicate process */
  switch (ms_mdre_pid) {
    case -1: /* error */
      printf("[%s] FATAL: fork for ms_mdre_ip failed\n",ME);
      exit(EXIT_FAILURE);
    case 0: /* fork() succeeded; we are now in the child process */
      err = execl("./ms_mdre_ip","ms_mdre_ip",NULL); /* launch ms_exec */
      /* if we get to this point then we failed to launch */
      if (err==-1) {
        printf("[%s] FATAL: failed to exec() ms_mdre_ip\n",ME); 
        }     
      exit(EXIT_FAILURE);
      break; 
    default: /* fork() succeeded; we are now in the parent process */
